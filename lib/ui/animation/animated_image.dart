...
class _AnimatedImageState extends State<AnimatedImage> with TickerProviderStateMixin {
  ...
  _AnimatedImageState({@required int imageCount, @required Duration duration, @required this.animationRootFolder, @required this.animationImagePrefix}) {
    ...
    _animation = new IntTween(begin: 0, end: imageCount - 1).animate(_controller);
  }

  void _preload(int imageCount) {
    for (int i = 1; i <= imageCount; i++) {
      String frame = i.toString().padLeft(4, '0');
      images.add(new Image.asset("$animationRootFolder/$animationImagePrefix$frame.png", gaplessPlayback: true));
    }
  }

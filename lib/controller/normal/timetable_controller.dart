...
class NormalTimetableController implements TimetableController {
  ...
  Future<bool> addCustomCourse(CustomCourse course) async {
    bool result = 0 != await cacheCustomCourseProvider.putCourse(course, new Injector().sessionController.user);
  ...
  Future<bool> removeCustomCourse(CustomCourse course) async {
    bool result = await cacheCustomCourseProvider.deleteCourse(course, new Injector().sessionController.user);
    timetable.then((timetable) => observers
        .forEach((observer) => observer.onFavoritesUpdated(timetable)));
    return result;
  }
  ...
  Future<List<CustomCourse>> get timetable async {
    List<CustomCourse> tempList =
        (await cacheSelectedCourseProvider.getCourses(new Injector().sessionController.user))
            .map((course) => course.toCustomCourse())
            .toList();
    tempList.addAll(await cacheCustomCourseProvider.getCourses(new Injector().sessionController.user));

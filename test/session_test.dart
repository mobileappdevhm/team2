...
import 'package:courses_in_english/connect/dataprovider/mockito_provider_factory.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    when(providerFactory.lecturerProvider.getLecturers()).thenAnswer((_) => new Future<List<Lecturer>>(() {}));
    when(providerFactory.departmentProvider.getDepartments()).thenAnswer((_) => new Future<List<Department>>(() {}));
    when(providerFactory.courseProvider.getFavorizedCourses()).thenAnswer((_) => new Future<List<Course>>(() {}));
    when(providerFactory.courseProvider.getSelectedCourses()).thenAnswer((_) => new Future<List<Course>>(() {}));
    when(providerFactory.courseProvider.getCourses()).thenAnswer((_) => new Future<List<Course>>(() {}));
    when(providerFactory.campusProvider.getCampuses()).thenAnswer((_) => new Future<List<Campus>>(() {}));

    when(providerFactory.courseProvider.favorizeCourse(course01)).thenAnswer((_) => new Future(() => true));
    when(providerFactory.courseProvider.unFavorizeCourse(course01)).thenAnswer((_) => new Future(() => false));

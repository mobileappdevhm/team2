...
void main() {
  ...
  testWidgets("Test Password Reset Layout", (WidgetTester tester) async {
    await tester
        .pumpWidget(new MaterialApp(home: new PasswordReset("abc1@hm.edu")));
    expect(find.byType(TextFormField), findsNWidgets(4));
    expect(
        find.widgetWithText(RoundedButton, "Reset Password"), findsOneWidget);
  });

  testWidgets("Test Password Reset", (WidgetTester tester) async {
    ...
    await tester.pump();
    verify(inetFactory.userProvider
            .resetPassword('abc1@hm.edu', 'code', 'newPW'))
        .called(1);
    expect(find.widgetWithText(AlertDialog, "Password Successfully changed!"),
        findsOneWidget);
    expect(find.widgetWithText(FlatButton, "Return to Login Page"),
        findsOneWidget);
  });

  testWidgets("Test Password Reset - Error", (WidgetTester tester) async {
    when(inetFactory.userProvider.resetPassword('abc1@hm.edu', 'code', 'newPW'))
    ...
    expect(
        find.widgetWithText(
            AlertDialog, "There was an Error changing the Password"),
        findsOneWidget);
    expect(find.widgetWithText(FlatButton, "Close"), findsOneWidget);
  });

  testWidgets("Test Password Reset Invalid Input - no Email",
    ...
    await tester.tap(find.byType(RoundedButton).first);
    await tester.pump();
    expect(find.widgetWithText(SnackBar, "Please fill all Input Fields"),
        findsOneWidget);
  });
  ...
  testWidgets("Test Password Reset Invalid Input - email no @",
      (WidgetTester tester) async {
    await tester
        .pumpWidget(new MaterialApp(home: new PasswordReset('abc1hm.edu')));
    await tester.enterText(find.byType(TextFormField).at(1), 'code');
    await tester.enterText(find.byType(TextFormField).at(2), 'newPW');
    await tester.enterText(find.byType(TextFormField).at(3), 'newPW');
    await tester.tap(find.byType(RoundedButton).first);
    await tester.pump();
    expect(find.widgetWithText(SnackBar, "Please enter data in a valid format"),
        findsOneWidget);
  });
  ...
  testWidgets("Test Password Reset Invalid Input - passwords don't match",
      (WidgetTester tester) async {
    await tester
        .pumpWidget(new MaterialApp(home: new PasswordReset('abc1@hm.edu')));
    await tester.enterText(find.byType(TextFormField).at(1), 'code');
    ...
    await tester.pump();
    expect(
        find.widgetWithText(SnackBar, "Passwords must match!"), findsOneWidget);
  });

  testWidgets("Test Password Reset Invalid Input - passwords don't match 2",
      (WidgetTester tester) async {
    await tester
        .pumpWidget(new MaterialApp(home: new PasswordReset('abc1@hm.edu')));
    await tester.enterText(find.byType(TextFormField).at(1), 'code');
  ...
  testWidgets("Test Password Reset Invalid Input - password to short",
      (WidgetTester tester) async {
    await tester
        .pumpWidget(new MaterialApp(home: new PasswordReset('abc1@hm.edu')));
    await tester.enterText(find.byType(TextFormField).at(1), 'code');
    ...
    await tester.tap(find.byType(RoundedButton).first);
    await tester.pump();
    expect(find.widgetWithText(SnackBar, "Password is to Short (min. 5)"),
        findsOneWidget);
  });
}
